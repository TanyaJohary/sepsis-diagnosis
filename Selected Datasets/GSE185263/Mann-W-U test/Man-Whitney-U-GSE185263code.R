...
# Save results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_all_GSE185263.csv", row.names = FALSE)
...
if (nrow(significant_genes) > 0) {
  ...
  top_25_genes <- significant_genes %>% slice_min(order_by = adjusted_p, n = 10)
  ordered_genes <- top_25_genes$Gene
  
  # Reshape the data into long format for plotting
  data_long <- data %>%
    dplyr::select(Label, all_of(ordered_genes)) %>%
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    geom_jitter(width = 0.2, size = 0.8, alpha = 0.2) + # Add jittered points for visibility
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_all_GSE185263.png", plot = p, width = 14, height = 10, dpi = 300)
