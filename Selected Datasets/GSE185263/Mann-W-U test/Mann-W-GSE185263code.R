...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_GSE185263.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_GSE185263.csv")
write_csv(non_significant_genes, "non_significant_genes_GSE185263.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("sepsis" = "#59E3AC", "healthy" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_10_significant_genes_boxplot_GSE185263.png",
...
volcano_df <- mann_whitney_results %>%
  mutate(
...
write.csv(mann_whitney_results_collapsed, "mann_whitney_results_adjusted_GSE185263.csv", row.names = FALSE)
