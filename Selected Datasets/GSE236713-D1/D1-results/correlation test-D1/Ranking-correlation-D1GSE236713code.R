...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("~/sepsis/MWU-tops/mann_whitney_results_adjusted_D1_GSE236713.csv")
rf_importance        <- read_csv("~/sepsis/average-feature-importance/average_feature_importance_SMOTE_GSE235713.csv")

data <- read.csv("sepsis_D1_dataGSE236713.csv")
...
write_csv(mann_whitney_ranking, "mann-w-rankingD1GSE136713.csv")
...
write_csv(rf_ranking, "rf-ranking-D1GSE136713.csv")
###############################################################################
...
write_csv(merged_ranking, "merged_rankings_comparison_D1GSE136713.csv")
...
write_csv(cor_test_results, "correlation_test_results_D1GSE136713.csv")
...
ggsave("comparison_ranks_D1GSE136713.png", p_comparison, width = 6, height = 5, dpi = 300)
print(p_comparison)
...
ggsave("hist_rank_diff_D1GSE136713.png", p_hist_diff, width = 6, height = 4, dpi = 300)
print(p_hist_diff)
...
ggsave("bar_top_discrepancies_D1GSE136713.png", p_bar_diff, width = 6, height = 4, dpi = 300)
print(p_bar_diff)
...
ggsave("hist_mannwhitney_pvals_D1GSE136713.png", p_hist_pval, width = 6, height = 4, dpi = 300)
print(p_hist_pval)
...
# Ensure adjusted_p is numeric (to avoid errors in lm model)
merged_ranking <- merged_ranking %>%
  mutate(adjusted_p = as.numeric(adjusted_p))
...
# (c) Define our criteria for highlighting genes
...
p_scatter_pval_importance_labeled <- ggplot(merged_ranking, aes(x = adjusted_p, y = MeanImportance)) +
  ...
  labs(
    title = "Mann–Whitney p-values vs. Random Forest Importance",
...
# (e) Save the plot
ggsave("scatter_pval_importance_labeled_D1GSE136713.png", p_scatter_pval_importance_labeled, width = 7, height = 5, dpi = 300)
...
long_data <- data %>%
  mutate(Label = factor(Label, levels = c("Control", "Sepsis")))  # Ensure order is correct
...
effect_sizes <- effect_sizes %>%
  mutate(EffectSize = -EffectSize)  # Flip sign
...
# (c) Save final table
write.table(final_merged, "final_merged_with_effect_sizes_D1GSE136713.csv", row.names = FALSE, quote = FALSE)
...
ggsave("venn_top10_mw_rf_D1GSE136713.png", p_venn, width = 5, height = 4, dpi = 300)
print(p_venn)
...
write.csv(data.frame(OverlapGenes = overlap_genes),
          "top10_overlap_genes_D1GSE136713.csv",
          row.names = FALSE)
...
write.csv(boot_res$t, "boot_spearman_samples_D1GSE136713.csv", row.names=FALSE)
