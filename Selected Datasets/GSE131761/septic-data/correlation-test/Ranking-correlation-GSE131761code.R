...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("~/sepsis/MWU-tops/mann_whitney_results_adjusted_GSE131761.csv")
rf_importance        <- read_csv("~/sepsis/average-feature-importance/average_feature_importance_SMOTE_GSE131761.csv")
...
write_csv(mann_whitney_ranking, "mann-w-rankingGSE131761.csv")
...
write_csv(rf_ranking, "rf-ranking-GSE131761.csv")
###############################################################################
...
write_csv(merged_ranking, "merged_rankings_comparison_GSE131761.csv")
...
write_csv(cor_test_results, "correlation_test_results_GSE131761.csv")
...
ggsave("comparison_ranks_GSE131761.png", p_comparison, width = 6, height = 5, dpi = 300)
print(p_comparison)
...
ggsave("hist_rank_diff_GSE131761.png", p_hist_diff, width = 6, height = 4, dpi = 300)
print(p_hist_diff)
...
ggsave("bar_top_discrepancies_GSE131761.png", p_bar_diff, width = 6, height = 4, dpi = 300)
print(p_bar_diff)
...
ggsave("hist_mannwhitney_pvals_GSE131761.png", p_hist_pval, width = 6, height = 4, dpi = 300)
print(p_hist_pval)
...
# (e) Save the plot
ggsave("scatter_pval_importance_labeled_GSE131761.png", p_scatter_pval_importance_labeled, width = 7, height = 5, dpi = 300)
...
long_data <- data %>%
  mutate(Label = factor(Label, levels = c("control patient", "septic shock")))  # Ensure order is correct
...
# (c) Save final table
write.table(final_merged, "final_merged_with_effect_sizes_GSE131761.csv", row.names = FALSE, quote = FALSE)
...
ggsave("venn_top10_mw_rf_GSE131761.png", p_venn, width = 5, height = 4, dpi = 300)
print(p_venn)
...
write.csv(data.frame(OverlapGenes = overlap_genes),
          "top10_overlap_genes_GSE131761.csv",
          row.names = FALSE)
...
write.csv(boot_res$t, "boot_spearman_samples_GSE131761.csv", row.names=FALSE)
