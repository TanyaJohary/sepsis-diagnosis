...
# Save results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_all_GSE154918.csv", row.names = FALSE)
...
write_csv(significant_genes, "significant_genes_all_GSE154918.csv")
write_csv(non_significant_genes, "non_significant_genes_all_GSE154918.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("Sepsis" = "#F75C71", "Healthy control" = "grey")) +  # Custom colors
    labs(
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_all_GSE154918.png", plot = p, width = 14, height = 10, dpi = 300)
