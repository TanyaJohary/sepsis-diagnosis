...
###############################################################################
data <- read.csv("sepsis_dataGSE241238.csv")

# Remove 'Sample' column and ensure 'Label' is factor
data <- dplyr::select(data, -Sample)
data$Label <- factor(data$Label, levels = c("Healthy","Sepsis"))  # consistent factor levels
...
for (i in seq_len(n_repeats)) {
  all_partitions[[i]] <- createDataPartition(data$Label, p = 0.7, list = FALSE)
}

###############################################################################
# 2) Baseline Random Forest (No SMOTE)
...
results_list <- foreach(i = seq_len(n_repeats),
                        .packages = c("randomForest","caret","pROC","dplyr")) %dopar% {
                          ...
                          # Factor levels
                          train_data$Label <- factor(train_data$Label, levels = c("Healthy","Sepsis"))
                          test_data$Label  <- factor(test_data$Label, levels = c("Healthy","Sepsis"))
                          ...
                          # Check if we have at least 1 Sepsis and 1 Healthy
                          n_Sepsis  <- sum(train_data$Label == "Sepsis")
                          n_Healthy <- sum(train_data$Label == "Healthy")
                          
                          if (n_Sepsis == 0 || n_Healthy == 0) {
                            # In principle, this is redundant given the check above, but let's keep it
                            return(list(
                              metrics            = c(NA, NA, NA, NA, NA, NA, NA),
                              feature_importance = NULL,
                              skipped            = TRUE,
                          ...
                          # Train Random Forest directly on train_data (No SMOTE)
                          ...
                          preds <- ifelse(probs > 0.5, "Sepsis", "Healthy")
                          
                          # If test set or predictions have only one class, skip
                          ...
                          confusion <- confusionMatrix(
                            data      = factor(preds, levels = c("Healthy","Sepsis")),
...
cat("\nNumber of skipped iterations:", skipped_count, "out of", n_repeats, "\n")

# If iteration #1 was not skipped, sample_conf_mat holds the entire confusionMatrix
# (with overall + byClass stats)
if (!is.null(results_list[[1]]$full_conf_obj)) {
  sample_conf_mat <- results_list[[1]]$full_conf_obj
...
###############################################################################
# 2c) Save and Summarize Baseline Results
###############################################################################
write.csv(results, "repeated_splits_GSE241238.csv", row.names = FALSE)

avg_metrics <- colMeans(results, na.rm = TRUE)
sd_metrics  <- apply(results, 2, sd, na.rm = TRUE)

cat("\nBaseline (All Features) Average Metrics:\n")
print(avg_metrics)
...
write.csv(average_metrics_df, "average_metrics_GSE241238.csv", row.names = FALSE)

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_GSE241238.csv", row.names = FALSE)
...
write.csv(avg_importance, "average_feature_importance_GSE241238.csv", row.names = FALSE)

###############################################################################
# 2d) Plot Feature Importance (with error bars)
###############################################################################
top_n_features <- 50
...
feature_plot <- ggplot(
  ...
  labs(
    ...
    subtitle = "Random Forest (No SMOTE)",
    x        = "Feature",
    y        = "Importance",
    fill     = "Importance"
  ) +
...
ggsave("top_feature_importance_plot_GSE241238.png", 
       plot = feature_plot, width = 12, height = 7, dpi = 300)

###############################################################################
# 3) Feature Removal Analysis (No SMOTE)
###############################################################################
...
for (f_idx in seq_along(features)) {
  ...
  removal_list <- foreach(i = seq_len(n_repeats),
                          .packages = c("randomForest","caret","pROC")) %dopar% {
                            ...
                            # If only one class in train, skip
                            if (length(unique(train_data$Label)) < 2) {
                              return(rep(NA,7))
                            }
...
write.csv(feature_removal_results, "feature_removal_results_GSE241238.csv", row.names = FALSE)
...
mcc_plot <- ggplot(mcc_comparison_df, aes(x = Feature, y = MCC, fill = MCC)) +
  ...
  coord_flip() +
  scale_fill_gradient(low = "#FF514E", high = "#138A78") +
  theme_light(base_size = 14) +
  labs(
...
ggsave("feature_removal_mcc_plot_GSE241238.png",
       plot = mcc_plot, width = 10, height = 12, dpi = 300)

###############################################################################
# 4) Sanity Check: Adding Noise + Using Same Partitions (No SMOTE)
###############################################################################
...
for (noise_level in noise_levels) {
  ...
  noise_results_list <- foreach(i = seq_len(n_repeats),
                                .packages = c("randomForest","caret","pROC")) %dopar% {
                                  ...
                                  # Train RF directly on noisy train data
                                  rf_model <- randomForest(
                                    Label ~ .,
                                    data  = train_data,
                                    ntree = 100,
...
write.csv(sanity_check_results, "sanity_check_results_GSE241238.csv", row.names = FALSE)
...
noise_plot <- ggplot(sanity_check_results, aes(x = Noise_Level)) +
  ...
  labs(
    title    = "Impact of Noise on Model Performance (No SMOTE)",
...
ggsave("noise_plot_GSE241238.png",
