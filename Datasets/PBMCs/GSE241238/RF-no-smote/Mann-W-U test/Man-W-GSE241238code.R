...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_GSE241238.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_GSE241238.csv")
write_csv(non_significant_genes, "non_significant_genes_GSE241238.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  top_25_genes <- significant_genes %>%
    slice_min(order_by = adjusted_p, n = 16, with_ties = FALSE)
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    facet_wrap(~ Gene_label, scales = "free_y", ncol = 4) + # 5 columns of facets
    scale_fill_manual(values = c("Sepsis" = "#FFC646", "Healthy" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_GSE241238.png",
