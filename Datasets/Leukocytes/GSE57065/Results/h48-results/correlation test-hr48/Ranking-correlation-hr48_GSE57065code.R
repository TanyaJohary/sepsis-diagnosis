...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("mann_whitney_results_adjusted_hr48_GSE57065.csv")
rf_importance        <- read_csv("average_feature_importance_noSMOTE_h48_GSE57065.csv")

data <- read.csv("sepsis_hr48_GSE57065.csv")
data$Label <- factor(data$Label, levels = c("Healthy", "Septic"))
...
write_csv(mann_whitney_ranking, "mann-w-ranking_hr48_GSE57065.csv")
...
write_csv(rf_ranking, "rf-ranking-hr48_GSE57065.csv")
###############################################################################
...
write_csv(merged_ranking, "merged_rankings_comparison_hr48_GSE57065.csv")
...
write_csv(cor_test_results, "correlation_test_results_hr48_GSE57065.csv")
...
ggsave("comparison_ranks_hr48_GSE57065.png", p_comparison, width = 6, height = 5, dpi = 300)
print(p_comparison)
...
ggsave("hist_rank_diff_hr48_GSE57065.png", p_hist_diff, width = 6, height = 4, dpi = 300)
print(p_hist_diff)
...
ggsave("bar_top_discrepancies_hr48_GSE57065.png", p_bar_diff, width = 6, height = 4, dpi = 300)
print(p_bar_diff)
...
ggsave("hist_mannwhitney_pvals_hr48_GSE57065.png", p_hist_pval, width = 6, height = 4, dpi = 300)
print(p_hist_pval)
...
ggsave("scatter_pval_importance_labeled_hr48_GSE57065.png", p_scatter_pval_importance_labeled, width = 7, height = 5, dpi = 300)
...
long_data <- data %>%
  pivot_longer(
    cols = where(is.numeric),  # Select only numeric columns to pivot
    names_to = "Gene",
...
# (c) Save final table
write_csv(final_merged, "final_merged_with_effect_sizes_hr48_GSE57065.csv")
...
ggsave("venn_top10_mw_rf_hr48_GSE57065.png", p_venn, width = 5, height = 4, dpi = 300)
print(p_venn)
...
write.csv(data.frame(OverlapGenes = overlap_genes),
          "top10_overlap_genes_hr48_GSE57065.csv",
          row.names = FALSE)
...
write.csv(boot_res$t, "boot_spearman_samples_hr48_GSE57065.csv", row.names=FALSE)
