...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_h24_h24_GSE57065.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_h24_GSE57065.csv")
write_csv(non_significant_genes, "non_significant_genes_h24_GSE57065.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  top_25_genes <- significant_genes %>%
    slice_min(order_by = adjusted_p, n = 20, with_ties = FALSE)
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("Septic" = "#F58371", "Healthy" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_h24_GSE57065.png",
