...
###############################################################################
data <- read.csv("sepsis_dataGSE6535.csv")

# Remove 'Sample' column and ensure 'Label' is factor
data <- dplyr::select(data, -Sample)
data$Label <- factor(data$Label, levels = c("Control","Sepsis"))  # consistent factor levels
...
results_list <- foreach(i = seq_len(n_repeats), 
                        .packages = c("randomForest", "smotefamily", "caret", "pROC", "dplyr")) %dopar% {
                          ...
                          # Factor levels
                          train_data$Label <- factor(train_data$Label, levels = c("Control", "Sepsis"))
                          test_data$Label  <- factor(test_data$Label, levels = c("Control", "Sepsis"))
                          ...
                          n_Healthy <- sum(train_data$Label == "Control")
                          ...
                          # Compute SMOTE duplication factor.
                          # Using floor() gives a less aggressive oversampling.
                          if (n_Sepsis < n_Healthy) {
                            # Sepsis is minority; we want to oversample Sepsis
                            dup_size_value <- as.integer(floor((n_Healthy / n_Sepsis) - 1))
                          } else if (n_Healthy < n_Sepsis) {
                            # Control is minority; oversample Control
                            dup_size_value <- as.integer(floor((n_Sepsis / n_Healthy) - 1))
                          } else {
                            dup_size_value <- 0  # Already balanced
                          }
                          ...
                          smote_train_data$Label <- factor(smote_train_data$Label, levels = c("Control", "Sepsis"))
                          
                          # Capture SMOTE distribution (after oversampling)
                          smote_dist <- table(smote_train_data$Label)
                          
                          # Optional: If we require an exactly balanced training set,
                          # we can undersample the majority class as follows:
                          if(length(unique(smote_dist)) > 1) {
                            target_count <- min(smote_dist)
                            smote_train_data <- do.call(rbind, lapply(levels(smote_train_data$Label), function(cls) {
                              subset <- smote_train_data[smote_train_data$Label == cls, ]
                              if(nrow(subset) > target_count) {
                                subset <- subset[sample(1:nrow(subset), target_count), ]
                              }
                              subset
                            }))
                            smote_train_data$Label <- factor(smote_train_data$Label, levels = c("Control", "Sepsis"))
                            smote_dist <- table(smote_train_data$Label)  # update distribution
                          }
                          ...
                          preds <- ifelse(probs > 0.5, "Sepsis", "Control")
                          ...
                          confusion <- confusionMatrix(
                            data      = factor(preds, levels = c("Control", "Sepsis")),
                            reference = test_data$Label,
                            positive  = "Sepsis"
                          )
                          cm_table <- confusion$table
                          
                          # Only store the entire confusionMatrix for iteration #1
                          if (i == 1) {
...
###############################################################################
write.csv(results, "repeated_splits_SMOTE_GSE6535.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_GSE6535.csv", row.names = FALSE)

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_GSE6535.csv", row.names = FALSE)
...
write.csv(avg_importance, "average_feature_importance_SMOTE_GSE6535.csv", row.names = FALSE)
...
ggsave("top_feature_importance_plot_SMOTE_GSE6535.png", 
...
for (f_idx in seq_along(features)) {
  ...
  removal_list <- foreach(i = seq_len(n_repeats), 
                          .packages = c("randomForest","caret","smotefamily","pROC")) %dopar% {
                            ...
                            # Compute SMOTE duplication factor based on the minority class
                            if (n_Sepsis < n_Healthy) {
                              # Sepsis is the minority
                              dup_size_value <- as.integer(floor((n_Healthy / n_Sepsis) - 1))
                            } else if (n_Healthy < n_Sepsis) {
                              # Control is the minority
                              dup_size_value <- as.integer(floor((n_Sepsis / n_Healthy) - 1))
                            } else {
                              dup_size_value <- 0  # Already balanced
                            }
                            ...
                            # Optional: Undersample to achieve an exactly balanced training set
                            smote_dist <- table(smote_train_data$Label)
                            if (length(unique(smote_dist)) > 1) {
                              target_count <- min(smote_dist)
                              smote_train_data <- do.call(rbind, lapply(levels(smote_train_data$Label), function(cls) {
...
write.csv(feature_removal_results, "feature_removal_results_SMOTE_GSE6535.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot_SMOTE_GSE6535.png", 
...
for (noise_level in noise_levels) {
  ...
  noise_results_list <- foreach(i = seq_len(n_repeats), 
                                .packages = c("randomForest","caret","smotefamily","pROC")) %dopar% {
                                  ...
                                  dup_size_value <- max(
                                    as.integer(ceiling((n_Sepsis / n_Healthy) - 1)),
                                    0
                                  )
...
write.csv(sanity_check_results, "sanity_check_results_SMOTE_GSE6535.csv", row.names = FALSE)
