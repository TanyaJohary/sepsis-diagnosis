################################################################################
# Improved Code: Random Forest with Repeated Split and Feature Removal Analysis
################################################################################
...
data <- data %>%
  mutate(num_label = ifelse(Label == "Sepsis", 1, 0)) %>%
  dplyr::select(-Label)
...
for (i in 1:n_repeats) {
  ...
  train_index <- createDataPartition(data$num_label, p = 0.8, list = FALSE)
  train_data <- data[train_index, ]
  test_data <- data[-train_index, ]
  
  # Train Random Forest model
  rf_model <- randomForest(as.factor(num_label) ~ ., data = train_data, ntree = 100, mtry = sqrt(ncol(train_data) - 1))
  ...
  if (length(unique(test_data$num_label)) < 2 || length(unique(predictions)) < 2) {
    cat("Iteration", i, "skipped due to class imbalance in test set or predictions.\n")
    next
  }
  
  # Calculate metrics
  confusion <- confusionMatrix(factor(predictions, levels = c(0, 1)), 
                               factor(test_data$num_label, levels = c(0, 1)), positive = "1")
  TP <- confusion$table[2, 2]
  FP <- confusion$table[1, 2]
  FN <- confusion$table[2, 1]
  TN <- confusion$table[1, 1]
  AUC <- ifelse(length(unique(probs)) > 1, auc(test_data$num_label, probs), NA)
...
################################################################################
features <- setdiff(names(data), "num_label")
...
for (feature_to_remove in features) {
  ...
  for (i in 1:n_repeats) {
    train_index <- createDataPartition(modified_data$num_label, p = 0.8, list = FALSE)
...
for (noise_level in noise_levels) {
  ...
  for (i in 1:n_repeats) {
    train_index <- createDataPartition(noisy_data$num_label, p = 0.8, list = FALSE)
