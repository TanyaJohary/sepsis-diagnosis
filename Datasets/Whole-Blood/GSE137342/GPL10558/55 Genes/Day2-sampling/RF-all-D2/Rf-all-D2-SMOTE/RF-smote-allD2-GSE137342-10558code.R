...
################################################################################
# Repeated Train-Test Splits with Random Forest and SMOTE
################################################################################
n_repeats <- 100
results <- data.frame(
...
for (i in 1:n_repeats) {
  ...
  smote_train_data <- SMOTE(train_data[, -ncol(train_data)], train_data$Label, K = 5, dup_size = 2)
  ...
  rf_model <- randomForest(Label ~ ., data = smote_train_data, ntree = 100, mtry = sqrt(ncol(smote_train_data) - 1))
...
write.csv(results, "repeated_splits_metrics_SMOTE_allD2_137342.csv", row.names = FALSE)
# Average metrics
...
write.csv(average_metrics_df, "average_metrics_SMOTE_allD2_137342.csv", row.names = FALSE)

# Print confirmation
cat("Average metrics saved to 'average_metrics_SMOTE_allD2_137342.csv'\n")
...
write.csv(feature_removal_results, "feature_removal_results_SMOTE_allD2_137342.csv", row.names = FALSE)
...
ggplot(feature_removal_results, aes(x = reorder(Feature, -MCC), y = MCC)) +
  ...
  theme_minimal() +
  theme(axis.text = element_text(size = 6), axis.title = element_text(size = 14))
ggsave("feature_removal_mcc_plot_SMOTE_allD2_137342.png", width = 10, height = 12, dpi = 300)
...
for (noise_level in noise_levels) {
  ...
  if (noise_level > 0) {
    ...
    noisy_data[noisy_rows, -ncol(noisy_data)] <- noisy_data[noisy_rows, -ncol(noisy_data)] * 
      runif(n = num_noisy_rows * (ncol(data) - 1), min = 0.8, max = 1.2)
  }
...
write.csv(sanity_check_results, "sanity_check_results_SMOTE_allD2_137342.csv", row.names = FALSE)
...
noise_plot <- ggplot(sanity_check_results, aes(x = Noise_Level)) +
  ...
  geom_line(aes(y = TPR, color = "TPR (Sensitivity)"), linetype = "dotted", size = 1.2) +
  geom_line(aes(y = TNR, color = "TNR (Specificity)"), linetype = "dotted", size = 1.2) +
  ...
  scale_color_manual(values = c("MCC" = "blue", "F1 Score" = "green", "AUC" = "red", 
                                "TPR (Sensitivity)" = "purple", "TNR (Specificity)" = "orange"))

print(noise_plot)
ggsave("impact_of_noise_SMOTE_allD2_137342.png", plot = noise_plot, width = 10, height = 6, dpi = 300)
