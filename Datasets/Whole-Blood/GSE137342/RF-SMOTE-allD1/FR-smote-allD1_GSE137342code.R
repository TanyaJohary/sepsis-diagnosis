...
# Read in our data
data <- read.csv("sepsis_allD1_GSE137342_10558.csv")
...
for (i in 1:n_repeats) {
  ...
  # Convert Label column to a factor, ensuring both levels are always present
  ...
  if (length(unique(train_data$Label)) < 2) {
    cat("Skipping iteration", i, "due to missing class labels in training data.\n")
    next
  }
...
write.csv(results, "repeated_splits_SMOTE_allD1_GSE137342.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_allD1_GSE137342.csv", row.names = FALSE)
cat("Average metrics saved to 'average_metrics_SMOTE_allD1_GSE137342.csv'\n")

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_allD1_GSE137342.csv", row.names = FALSE)
cat("Feature importance results saved to 'feature_importance_SMOTE_allD1_GSE137342.csv'\n")
...
write.csv(avg_importance, "average_feature_importance_SMOTE_allD1_GSE137342.csv", row.names = FALSE)
cat("Average feature importance saved to 'average_feature_importance_SMOTE_allD1_GSE137342.csv'\n")
...
print(feature_plot)
ggsave("top_feature_importance_plot_SMOTE_allD1_GSE137342.png", plot = feature_plot, width = 12, height = 7, dpi = 300)
...
for (feature_to_remove in features) {
  ...
  for (i in 1:n_repeats) {
    ...
    # SMOTE Oversampling
    if (sum(train_data$Label == "Sepsis") == 0 || sum(train_data$Label == "Healthy") == 0) {
      cat("Skipping iteration due to missing class labels in training data.\n")
...
# Save results to CSV
write.csv(feature_removal_results, "feature_removal_results_SMOTE_allD1_GSE137342.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot_SMOTE_allD1_GSE137342.png", width = 10, height = 12, dpi = 300)
...
# Save results
write.csv(sanity_check_results, "sanity_check_results_SMOTE_allD1_GSE137342.csv", row.names = FALSE)
...
print(noise_plot)
ggsave("noise_plot_allD1_GSE137342.png", plot = noise_plot, width = 10, height = 6, dpi = 300)
