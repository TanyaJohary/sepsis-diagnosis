...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("mann_whitney_results_adjusted_allD1_GSE137342.csv")
rf_importance        <- read_csv("average_feature_importance_SMOTE_allD1_GSE137342.csv")

data <- read.csv("sepsis_allD1_GSE137342_10558.csv")
data$Label <- factor(data$Label, levels = c("Healthy", "Sepsis"))
...
write_csv(mann_whitney_ranking, "mann-w-ranking_allD1_GSE137342.csv")
...
write_csv(rf_ranking, "rf-ranking-allD1_GSE137342.csv")
###############################################################################
...
write_csv(merged_ranking, "merged_rankings_comparison_allD1_GSE137342.csv")
...
write_csv(cor_test_results, "correlation_test_results_allD1_GSE137342.csv")
...
ggsave("comparison_ranks_allD1_GSE137342.png", p_comparison, width = 6, height = 5, dpi = 300)
print(p_comparison)
...
ggsave("hist_rank_diff_allD1_GSE137342.png", p_hist_diff, width = 6, height = 4, dpi = 300)
print(p_hist_diff)
...
ggsave("bar_top_discrepancies_allD1_GSE137342.png", p_bar_diff, width = 6, height = 4, dpi = 300)
print(p_bar_diff)
...
ggsave("hist_mannwhitney_pvals_allD1_GSE137342.png", p_hist_pval, width = 6, height = 4, dpi = 300)
print(p_hist_pval)
...
ggsave("scatter_pval_importance_labeled_allD1_GSE137342.png", p_scatter_pval_importance_labeled, width = 7, height = 5, dpi = 300)
...
long_data <- data %>%
  pivot_longer(
    cols = -c(Sample, Label, IFNA2),  # exclude the ID columns, IFNA1 has same value in all samples
    names_to = "Gene",
...
# (c) Save final table
write_csv(final_merged, "final_merged_with_effect_sizes_allD1_GSE137342.csv")
...
ggsave("venn_top10_mw_rf_allD1_GSE137342.png", p_venn, width = 5, height = 4, dpi = 300)
print(p_venn)
...
write.csv(data.frame(OverlapGenes = overlap_genes),
          "top10_overlap_genes_allD1_GSE137342.csv",
          row.names = FALSE)
...
write.csv(boot_res$t, "boot_spearman_samples_allD1_GSE137342.csv", row.names=FALSE)
