...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("mann_whitney_results_adjusted_GSE65682.csv")
rf_importance        <- read_csv("average_feature_importance_SMOTE_GSE65682.csv")
...
effect_sizes <- long_data %>%
  ...
  ungroup() %>%            # Convert rowwise_df back to a tibble
  dplyr::select(-res)      # Explicitly call dplyr's select function
...
# (c) Save final table
write_csv(final_merged, "final_merged_with_effect_sizes_GSE65682.csv")
