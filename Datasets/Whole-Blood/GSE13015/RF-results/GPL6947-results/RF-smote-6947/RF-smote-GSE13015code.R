...
###############################################################################
data <- read.csv("sepsis_limma_6947.csv")
...
results_list <- foreach(i = seq_len(n_repeats), 
                        .packages = c("randomForest","smotefamily","caret","pROC","dplyr")) %dopar% {
                          ...
                          K_val <- if(n_Healthy > 1) min(5, n_Healthy - 1) else 1
...
###############################################################################
write.csv(results, "repeated_splits_SMOTE_GSE13015.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_GSE13015.csv", row.names = FALSE)

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_GSE13015.csv", row.names = FALSE)
...
write.csv(avg_importance, "average_feature_importance_SMOTE_GSE13015.csv", row.names = FALSE)
...
ggsave("top_feature_importance_plot_SMOTE_GSE13015.png", 
...
write.csv(feature_removal_results, "feature_removal_results_SMOTE_GSE13015.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot_SMOTE_GSE13015.png", 
...
write.csv(sanity_check_results, "sanity_check_results_SMOTE_GSE13015.csv", row.names = FALSE)
...
ggsave("noise_plot_GSE13015.png", 
