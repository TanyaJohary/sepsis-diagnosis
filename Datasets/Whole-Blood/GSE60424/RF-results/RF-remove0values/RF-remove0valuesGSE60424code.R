...
################################################################################
data <- read.csv("sepsis_dataGSE60424.csv")
data <- dplyr::select(data, -c("Sample", "CALCA", "VCAM1", "LBP", "CRP", "IFNB1", "CCL19", "IFNA1", "IFNA2"))
data <- data %>%
...
write.csv(results, "repeated_splits_metrics60424.csv", row.names = FALSE)
# Average metrics
...
write.csv(average_metrics_df, "average_metrics604242.csv", row.names = FALSE)

# Print confirmation
cat("Average metrics saved to 'average_metrics60424.csv'\n")
###############################################################################
...
write.csv(feature_removal_results, "feature_removal_results604242.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot60424.png", width = 10, height = 10, dpi = 300)
...
write.csv(sanity_check_results, "sanity_check_results60424.csv", row.names = FALSE)
...
print(noise_plot)
ggsave("impact_of_noise_on_model_performance60424.png", plot = noise_plot, width = 10, height = 6, dpi = 300)
...
# Train the Random Forest model with importance enabled
rf_model <- randomForest(Label ~ ., data = train_data, importance = TRUE)

# Extract feature importance
importance <- importance(rf_model)

# Convert importance to a data frame for easier handling
importance_df <- as.data.frame(importance)

# Add a column with feature names
importance_df$Feature <- rownames(importance_df)

# Order the features by their importance score (e.g., MeanDecreaseGini)
ordered_importance <- importance_df[order(-importance_df$MeanDecreaseGini), ]

# View the ordered importance
