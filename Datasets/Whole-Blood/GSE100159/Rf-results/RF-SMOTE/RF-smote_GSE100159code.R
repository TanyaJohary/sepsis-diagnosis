...
for (i in 1:n_repeats) {
  ...
  dup_size_value <- max(
    as.integer(ceiling(
      (sum(train_data$Label == "Control") / sum(train_data$Label == "Sepsis")) - 1
    )),
    0
  )
...
cat("Average metrics saved to 'average_metrics_SMOTE_GSE100159.csv'\n")

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_GSE100159.csv", row.names = FALSE)
cat("Feature importance results saved to 'feature_importance_SMOTE_GSE100159.csv'\n")
...
cat("Average feature importance saved to 'average_feature_importance_SMOTE_GSE100159.csv'\n")
...
print(feature_plot)
ggsave("top_feature_importance_plot_SMOTE_GSE100159.png", plot = feature_plot, width = 12, height = 7, dpi = 300)
...
ggsave("feature_removal_mcc_plot_SMOTE_GSE100159.png", width = 10, height = 12, dpi = 300)
...
print(noise_plot)
ggsave("noise_plot_GSE100159.png", plot = noise_plot, width = 10, height = 6, dpi = 300)
