...
data <- read.csv("filtered_sepsis_dataGSE208581.csv")
data <- dplyr::select(data, -Sample)
data <- data %>% mutate(Label = as.factor(Label))  # Ensure Label is a factor
data$Label <- factor(data$Label, levels = c("SIRS-", "Sepsis+"))  # Explicitly set levels
...
for (i in 1:n_repeats) {
  ...
  test_data$Label <- factor(test_data$Label, levels = c("SIRS-", "Sepsis+"))
  ...
  if (length(unique(probs)) > 1) {
    AUC <- auc(response = test_data$Label, predictor = probs, levels = c("SIRS-", "Sepsis+"), direction = "<")
  } else {
...
data$Label <- factor(data$Label, levels = c("Sepsis+", "SIRS-"))
...
for (feature_to_remove in features) {
  ...
  for (i in 1:100) {
    ...
    if (length(unique(probs)) > 1) {
      roc_obj <- roc(test_data$Label, probs, levels = c("Sepsis+", "SIRS-"), direction = "<")
      auc_val <- auc(roc_obj)
    } else {
...
ggplot(feature_removal_results, aes(x = reorder(Feature, -MCC), y = MCC)) +
  ...
  theme_minimal()
ggsave("gene_removal_mcc_plot.png", width = 10, height = 6, dpi = 300)
