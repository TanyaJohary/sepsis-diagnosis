...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("mann_whitney_results_adjusted_all_GSE154918.csv")
rf_importance        <- read_csv("average_gini_importance_all_GSE154918.csv")

data <- read.csv("all_sepsis_labeledGSE154918.csv")
...
long_data <- data %>%
  pivot_longer(
    cols = -c(IFNA1, Label),  # exclude the ID columns, IFNA1 has same value in all samples
    names_to = "Gene",
...
# (c) Save final table
write_csv(final_merged, "final_merged_with_effect_sizes_GSE154918.csv")
