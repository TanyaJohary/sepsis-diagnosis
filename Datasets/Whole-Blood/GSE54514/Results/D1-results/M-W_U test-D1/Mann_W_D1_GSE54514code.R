...
# Save results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_D1_GSE54514.csv", row.names = FALSE)
...
write_csv(significant_genes, "significant_genes_D1_GSE54514.csv")
write_csv(non_significant_genes, "non_significant_genes_D1_GSE54514.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  top_25_genes <- significant_genes %>% slice_min(order_by = adjusted_p, n = 5)
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.7, width = 0.6) +  # Reduce opacity and remove outliers
    geom_jitter(width = 0.2, size = 1, alpha = 0.6) + # Add jittered points for visibility
    facet_wrap(~ Gene, scales = "free_y", ncol = 3) +  # **Arrange into 5 columns**
    scale_fill_manual(values = c("Sepsis" = "#FF7135", "healthy" = "grey")) +  # Custom colors
    labs(
    ...
    theme(
      ...
      plot.margin = margin(10, 15, 10, 15),  # Add more spacing around the plot
      panel.spacing = unit(1.5, "lines") 
    )
  
  # Display and save the plot
  print(p)
  ggsave("top_25_significant_genes_boxplot_D1_GSE54514.png", plot = p, width = 14, height = 10, dpi = 300)
