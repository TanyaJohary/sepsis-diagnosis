...
# Save results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_D2_GSE54514.csv", row.names = FALSE)
...
write_csv(significant_genes, "significant_genes_D2_GSE54514.csv")
write_csv(non_significant_genes, "non_significant_genes_D2_GSE54514.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    facet_wrap(~ Gene, scales = "free_y", ncol = 4) +  # **Arrange into 5 columns**
    scale_fill_manual(values = c("Sepsis" = "#8A0042", "healthy" = "grey")) +  # Custom colors
    labs(
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_D2_GSE54514.png", plot = p, width = 14, height = 10, dpi = 300)
