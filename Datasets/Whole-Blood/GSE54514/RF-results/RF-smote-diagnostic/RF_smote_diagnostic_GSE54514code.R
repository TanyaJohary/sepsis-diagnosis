...
# Read in your data
data <- read.csv("sepsis_diagnosis_dataGSE54514.csv")
...
write.csv(results, "repeated_splits_SMOTE_GSE54514.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_GSE54514.csv", row.names = FALSE)
cat("Average metrics saved to 'average_metrics_SMOTE_GSE54514.csv'\n")

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_GSE54514.csv", row.names = FALSE)
cat("Feature importance results saved to 'feature_importance_SMOTE_GSE54514.csv'\n")
...
write.csv(avg_importance, "average_feature_importance_SMOTE_GSE54514.csv", row.names = FALSE)
cat("Average feature importance saved to 'average_feature_importance_SMOTE_GSE54514.csv'\n")
...
feature_plot <- ggplot(top_features, aes(x = reorder(Feature, Mean_Importance), y = Mean_Importance, fill = Mean_Importance)) +
  ...
  geom_text(aes(label = round(Mean_Importance, 2)), hjust = -0.2, size = 5, color = "black") +  # Add labels to bars
...
print(feature_plot)
ggsave("top_feature_importance_plot_SMOTE_GSE54514.png", plot = feature_plot, width = 12, height = 7, dpi = 300)
...
# Save results to CSV
write.csv(feature_removal_results, "feature_removal_results_SMOTE_GSE54514.csv", row.names = FALSE)

###############################################################################
# 4) Compare "All Features" vs. "Removed One Feature" Using the Same Splits
###############################################################################
# We'll assume you have a variable 'average_metrics' from your baseline (all features) scenario:
...
ggsave("feature_removal_mcc_plot_SMOTE_GSE54514.png", width = 10, height = 12, dpi = 300)
...
# Save results
write.csv(sanity_check_results, "sanity_check_results_SMOTE_GSE54514.csv", row.names = FALSE)
...
print(noise_plot)
ggsave("noise_plot_GSE54514.png", plot = noise_plot, width = 10, height = 6, dpi = 300)
