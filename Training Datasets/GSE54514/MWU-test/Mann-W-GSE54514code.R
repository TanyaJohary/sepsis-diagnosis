...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_GSE54514.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_GSE54514.csv")
write_csv(non_significant_genes, "non_significant_genes_GSE54514.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("Sepsis" = "#4FEEFF", "healthy" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_10_significant_genes_boxplot_GSE54514.png",
...
write.csv(mann_whitney_results_collapsed, "mann_whitney_results_adjusted_GSE54514.csv", row.names = FALSE)
...
# Display and save
print(volcano_plot)
ggsave("volcano_plot_only_highly_labeled_GSE54514.png", volcano_plot,
