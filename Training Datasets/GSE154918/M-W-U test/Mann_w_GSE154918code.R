...
for (gene in gene_columns) {
  ...
  test_result <- wilcox.test(sepsis_vals, control_vals, exact = TRUE)
...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_GSE154918.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_GSE154918.csv")
write_csv(non_significant_genes, "non_significant_genes_GSE154918.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("Sepsis" = "#5A9960", "Healthy" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_GSE154918.png",
...
# Display and save
print(volcano_plot)
ggsave("volcano_plot_only_highly_labeled_GSE154918.png", volcano_plot,
