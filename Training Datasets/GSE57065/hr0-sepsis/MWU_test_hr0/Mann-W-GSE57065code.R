...
for (gene in gene_columns) {
  ...
  sepsis_vals  <- gene_data[data$Label == "Septic"]
...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_GSE57065.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_GSE57065.csv")
write_csv(non_significant_genes, "non_significant_genes_GSE57065.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("Septic" = "#4FEEFF", "Healthy" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_10_significant_genes_boxplot_GSE57065.png",
...
write.csv(mann_whitney_results_collapsed, "mann_whitney_results_adjusted_GSE57065.csv", row.names = FALSE)
...
# Display and save
print(volcano_plot)
ggsave("volcano_plot_only_highly_labeled_GSE57065.png", volcano_plot,
