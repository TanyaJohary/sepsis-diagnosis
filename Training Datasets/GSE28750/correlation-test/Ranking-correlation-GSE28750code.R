...
# 2. Read Input Data
###############################################################################
mann_whitney_results <- read_csv("~/sepsis/MWU-tops/mann_whitney_results_adjusted_GSE28750.csv")
rf_importance        <- read_csv("~/sepsis/average-feature-importance/average_feature_importance_SMOTE_GSE28750.csv")
...
write_csv(mann_whitney_ranking, "mann-w-rankingGSE28750.csv")
...
write_csv(rf_ranking, "rf-ranking-GSE28750.csv")
###############################################################################
...
write_csv(merged_ranking, "merged_rankings_comparison_GSE28750.csv")
...
write_csv(cor_test_results, "correlation_test_results_GSE28750.csv")
...
ggsave("comparison_ranks_GSE28750.png", p_comparison, width = 6, height = 5, dpi = 300)
print(p_comparison)
...
ggsave("hist_rank_diff_GSE28750.png", p_hist_diff, width = 6, height = 4, dpi = 300)
print(p_hist_diff)
...
ggsave("bar_top_discrepancies_GSE28750.png", p_bar_diff, width = 6, height = 4, dpi = 300)
print(p_bar_diff)
...
ggsave("hist_mannwhitney_pvals_GSE28750.png", p_hist_pval, width = 6, height = 4, dpi = 300)
print(p_hist_pval)
...
# (e) Save the plot
ggsave("scatter_pval_importance_labeled_GSE28750.png", p_scatter_pval_importance_labeled, width = 7, height = 5, dpi = 300)
...
# (c) Save final table
write.table(final_merged, "final_merged_with_effect_sizes_GSE28750.csv", row.names = FALSE, quote = FALSE)
...
ggsave("venn_top10_mw_rf_GSE28750.png", p_venn, width = 5, height = 4, dpi = 300)
print(p_venn)
...
write.csv(data.frame(OverlapGenes = overlap_genes),
          "top10_overlap_genes_GSE28750.csv",
          row.names = FALSE)
...
write.csv(boot_res$t, "boot_spearman_samples_GSE28750.csv", row.names=FALSE)
