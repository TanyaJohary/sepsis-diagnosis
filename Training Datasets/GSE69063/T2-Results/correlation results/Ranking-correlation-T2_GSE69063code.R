...
pacman::p_load(dplyr, readr, ggplot2, corrr, rcompanion, ggrepel, tidyr)

###############################################################################
# 2. Read Input Data
###############################################################################
# Change the filenames to match your actual files if needed.
mann_whitney_results <- read_csv("mann_whitney_results_adjusted_T2_GSE69063.csv")
rf_importance        <- read_csv("average_feature_importance_SMOTE_T2_GSE69063.csv")

# Wide data for effect sizes (one row per sample, multiple gene columns, plus a 'Label' column)
# Adjust filename as appropriate
data <- read.csv("T2_sepsis_dataGSE69063.csv")
...
write_csv(mann_whitney_ranking, "mann-w-rankingT2-GSE69063.csv")
...
write_csv(rf_ranking, "rf-rankingT2-GSE69063.csv")
###############################################################################
...
write_csv(merged_ranking, "merged_rankings_comparison_T2_GSE69063.csv")
...
write_csv(cor_test_results, "correlation_test_results.csv")
...
ggsave("comparison_ranks.png", p_comparison, width = 6, height = 5, dpi = 300)
print(p_comparison)
...
ggsave("hist_rank_diff.png", p_hist_diff, width = 6, height = 4, dpi = 300)
print(p_hist_diff)
...
ggsave("bar_top_discrepancies.png", p_bar_diff, width = 6, height = 4, dpi = 300)
print(p_bar_diff)
...
ggsave("hist_mannwhitney_pvals.png", p_hist_pval, width = 6, height = 4, dpi = 300)
print(p_hist_pval)

###############################################################################
# 11. Scatter Plot: p-value vs. RF Mean Importance
##############################################################################
###############################################################################
...
# (c) Define your criteria. Example: 
...
ggsave("scatter_pval_importance_labeled.png", p_scatter_pval_importance_labeled, width = 7, height = 5, dpi = 300)
print(p_scatter_pval_importance_labeled)

###############################################################################
# 13. (Optional) Effect Size Calculations (Using rcompanion::wilcoxonR)
###############################################################################
# We assume 'data' is in wide format: columns = [Sample, Label, OLFM4, NLRP3, ...].
# 1) Pivot from wide -> long to get columns: [Sample, Label, Gene, Expression].

long_data <- data %>%
  pivot_longer(
    cols = -c(Sample, Label),  # exclude the ID columns
    names_to = "Gene",
...
###############################################################################
# If you want to incorporate the effect sizes into your main merged_ranking table:
...
# (c) Save final table
write_csv(final_merged, "final_merged_with_effect_sizes.csv")
...
ggsave("venn_top10_mw_rf.png", p_venn, width = 5, height = 4, dpi = 300)
print(p_venn)
...
# You could also save these bootstrap results if desired
write.csv(boot_res$t, "boot_spearman_samples.csv", row.names=FALSE)
