...
for (gene in gene_columns) {
  ...
  control_vals <- gene_data[data$Label == "healthy control"]
...
# Save full results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_T0_GSE69063.csv", row.names = FALSE)
...
write_csv(significant_genes,"significant_genes_T0_GSE69063.csv")
write_csv(non_significant_genes, "non_significant_genes_T0_GSE69063.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("sepsis" = "#376299", "healthy control" = "grey")) +
    labs(
  ...
  print(p)
  ggsave("top_10_significant_genes_boxplot_T0_GSE69063.png",
...
# Display and save
print(volcano_plot)
ggsave("volcano_plot_only_highly_labeled_T0_GSE69063.png", volcano_plot,
