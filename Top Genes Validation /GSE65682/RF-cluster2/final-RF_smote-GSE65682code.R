...
###############################################################################
data <- read.csv("cluster2_GSE65682.csv")
...
results_list <- foreach(i = seq_len(n_repeats), 
                        .packages = c("randomForest","smotefamily","caret","pROC","dplyr")) %dopar% {
                          ...
                          smote_train_data$Label <- factor(smote_train_data$Label, levels = c("healthy","Sepsis"))
...
###############################################################################
write.csv(results, "repeated_splits_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)
...
write.csv(avg_importance, "average_feature_importance_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)
...
ggsave("top_feature_importance_plot_SMOTE_cluster2_GSE65682.png", 
...
write.csv(feature_removal_results, "feature_removal_results_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot_SMOTE_cluster2_GSE65682.png", 
...
write.csv(sanity_check_results, "sanity_check_results_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)
...
ggsave("noise_plot_cluster2_GSE65682.png", 
...
cat("\nAll done.\n")
################################################################################
################# Aggregate multi probes of the same gene ######################
###############################################################################
...
write.csv(gene_importance, "average_feature_importance_SMOTE_cluster2_GSE65682.csv", row.names = FALSE)
