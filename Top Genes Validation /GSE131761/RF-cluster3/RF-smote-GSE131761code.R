...
###############################################################################
data <- read.csv("cluster3_GSE131761.csv")

# Remove 'Sample' column and ensure 'Label' is factor
data <- dplyr::select(data, -Sample)
data$Label <- factor(data$Label, levels = c("control patient","septic shock"))  # consistent factor levels
...
results_list <- foreach(i = seq_len(n_repeats), 
                        .packages = c("randomForest","smotefamily","caret","pROC","dplyr")) %dopar% {
                          ...
                          # Factor levels
                          train_data$Label <- factor(train_data$Label, levels = c("control patient","septic shock"))
                          test_data$Label  <- factor(test_data$Label, levels = c("control patient","septic shock"))
                          ...
                          n_septic_shock  <- sum(train_data$Label == "septic shock")
                          n_control_patient <- sum(train_data$Label == "control patient")
                          
                          if (n_septic_shock == 0 || n_control_patient == 0) {
                            return(list(
                              metrics            = c(NA, NA, NA, NA, NA, NA, NA),
                              feature_importance = NULL,
                              skipped            = TRUE,
                          ...
                          # Compute SMOTE ratio
                          dup_size_value <- max(
                            as.integer(ceiling((n_septic_shock / n_control_patient) - 1)),
                            0
                          )
                          ...
                          smote_train_data$Label <- factor(smote_train_data$Label, levels = c("control patient","septic shock"))
                          ...
                          # Predict
                          probs <- predict(rf_model, newdata = test_data, type = "prob")[, "septic shock"]
                          preds <- ifelse(probs > 0.5, "septic shock", "control patient")
                          ...
                          confusion <- confusionMatrix(
                            data      = factor(preds, levels = c("control patient","septic shock")),
...
###############################################################################
write.csv(results, "repeated_splits_SMOTE_cluster3GSE131761.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_cluster3GSE131761.csv", row.names = FALSE)

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_cluster3GSE131761.csv", row.names = FALSE)
...
write.csv(avg_importance, "average_feature_importance_SMOTE_cluster3GSE131761.csv", row.names = FALSE)
...
ggsave("top_feature_importance_plot_SMOTE_cluster3GSE131761.png", 
...
write.csv(feature_removal_results, "feature_removal_results_SMOTE_cluster3GSE131761.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot_SMOTE_cluster3GSE131761.png", 
...
write.csv(sanity_check_results, "sanity_check_results_SMOTE_cluster3GSE131761.csv", row.names = FALSE)
...
ggsave("noise_plot_cluster3GSE131761.png", 
...
write.csv(gene_importance, "average_feature_importance_SMOTE_cluster3GSE131761.csv", row.names = FALSE)
