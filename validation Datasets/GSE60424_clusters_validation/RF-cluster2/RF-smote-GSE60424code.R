...
###############################################################################
data <- read.csv("sepsis_cluster2_GSE60424.csv")

# Remove 'Sample' column and ensure 'Label' is factor
data <- dplyr::select(data, -Sample)
data$Label <- factor(data$Label, levels = c("Healthy_Control","Sepsis"))  # consistent factor levels
...
results_list <- foreach(i = seq_len(n_repeats), 
                        .packages = c("randomForest","smotefamily","caret","pROC","dplyr")) %dopar% {
                          ...
                          # Factor levels
                          train_data$Label <- factor(train_data$Label, levels = c("Healthy_Control","Sepsis"))
                          test_data$Label  <- factor(test_data$Label, levels = c("Healthy_Control","Sepsis"))
                          ...
                          n_Healthy <- sum(train_data$Label == "Healthy_Control")
                          ...
                          smote_train_data$Label <- factor(smote_train_data$Label, levels = c("Healthy_Control","Sepsis"))
                          ...
                          preds <- ifelse(probs > 0.5, "Sepsis", "Healthy_Control")
                          ...
                          confusion <- confusionMatrix(
                            data      = factor(preds, levels = c("Healthy_Control","Sepsis")),
...
###############################################################################
write.csv(results, "repeated_splits_SMOTE_cluster2-GSE60424.csv", row.names = FALSE)
...
write.csv(average_metrics_df, "average_metrics_SMOTE_cluster2-GSE60424.csv", row.names = FALSE)

# Save feature importance data
write.csv(all_feature_importance, "feature_importance_SMOTE_cluster2-GSE60424.csv", row.names = FALSE)
...
write.csv(avg_importance, "average_feature_importance_SMOTE_cluster2-GSE60424.csv", row.names = FALSE)
...
ggsave("top_feature_importance_plot_SMOTE_cluster2-GSE60424.png", 
...
write.csv(feature_removal_results, "feature_removal_results_SMOTE_cluster2-GSE60424.csv", row.names = FALSE)
...
ggsave("feature_removal_mcc_plot_SMOTE_cluster2-GSE60424.png", 
...
write.csv(sanity_check_results, "sanity_check_results_SMOTE_cluster2-GSE60424.csv", row.names = FALSE)
...
noise_plot <- ggplot(sanity_check_results, aes(x = Noise_Level)) +
  ...
  theme(
    plot.title    = element_text(size = 16, face = "bold", hjust = 0.5),
    plot.subtitle = element_text(size = 14, hjust = 0.5, color = "#1B2324"),
    axis.title.x  = element_text(size = 14, face = "bold"),
    axis.title.y  = element_text(size = 14, face = "bold"),
    axis.text.x   = element_text(size = 12),
...
ggsave("noise_plot_cluster2-GSE60424.png", 
