...
# Save results
write.csv(mann_whitney_results, "mann_whitney_results_adjusted_GSE211210.csv", row.names = FALSE)
...
write_csv(significant_genes, "significant_genes_GSE211210.csv")
write_csv(non_significant_genes, "non_significant_genes_GSE211210.csv")
...
if (nrow(significant_genes) > 0) {
  ...
  p <- ggplot(data_long, aes(x = Label, y = Expression, fill = Label)) +
    ...
    scale_fill_manual(values = c("Sepsis" = "#FF7135", "Control" = "grey")) +  # Custom colors
    labs(
  ...
  print(p)
  ggsave("top_25_significant_genes_boxplot_gse211210.png", plot = p, width = 14, height = 10, dpi = 300)
